...
begin
	plot(sin, 0:0.25:2π, title = "plot con atributo 'marker = true'", xlabel = "x", ylabel = L"sin(x)", color = "darkorange", label = "plot", marker = true, legend = false)
end
...
# ╔═╡ 47f759a2-e4c3-43d9-b274-b3446c24a7bb
begin
	plot(sin, -π:0.25:π, title = "Funciones trigonométricas", color = "pink", label = "sin(x)", marker = true)
	plot!(cos, -π:0.25:π, color = "lightblue", label = "cos(x)", marker = true)
end
...
# ╔═╡ 44def69b-3341-485f-a348-fa96a738426a
begin
	function primera!(a, r, θ, t)
	 a=1
	 r=3
	 θ=180
	 t=60
	
p(t)=(r*cosd(θ)*t,a+(r*t*sind(θ))-(9.81*(t^2)*(1/2)))    #La ecuación de x es x=Vo*t*cos(θ) 
												         #La ecuacion de y es h+Vo*t*sen(θ)-1/2*g*t^2
#sin y cos tienen una d porque los quiero en grados

tp = @animate for t in 0:1:100
	plot(p.(range(0,100,step=1)), legend=false, title="Tiro parabólico", lw=5, ls=:dot,color = :green)
	end
	gif(tp, "Tiroparabólico.gif", fps=30)
end
end

# ╔═╡ e84c1e2c-9977-4fdd-8ebc-1eb92631c328
begin 
	function movimientoparabolico(a, r, θ, t)
	primera!(1, 3, 180, 60)
	 a=1
	 r=3
	 θ=180
	...
	xmax=(((r^2)*sind(2θ))/9.81) #xmax=Vo^2sin(2θ)/g
	ymax=(((r^2)*(sind(θ))^2)/(2*9.81))#xmax=Vo^2sin(2θ)/g
    p(t)=(r*cosd(θ)*t,a+(r*t*sind(θ))-(9.81*(t^2)*(1/2)))

	xlabel!("x")                                  #En esta parte estoy tratando de que la animación se detenga justo cuando x es maxima 												así como y es maximq
	ylabel!("y")
	xlims!(xmax,0) 
	ylims!(0,ymax)
		pt = @animate for t in 0:1:100
	plot(p.(range(0,100,step=1)), legend=false, title="Tiro parabólico", lw=5, ls=:dot,color = :lighttest)
	end
	gif(pt, "Tiroparabólico.gif", fps=30)
end
end
...
movimientoparabolico(1, 3, 180, 60)

# ╔═╡ c8e1f100-e377-4680-ae64-db11f0c789b1
...
# ╔═╡ 0869314f-69cc-476c-bce0-991074e95397
movimientoparabolico(1, 3, 180, 60)
...
# ╔═╡ dd1727a4-6756-4899-9b71-7359c3b94672
surface(R,R,h, color = :lighttest)

# ╔═╡ ff25c787-a76f-4bd5-9cd2-8bc7b511f5c0
begin
	x=0:0.05:4π
	y=x

@gif for t in 0:0.05:4π
	surface(x,y,h,title=L"cos(x)+sin(y)",color=:lighttest, size=(500,200),rotate=(180,180,180), camera=(t,20))
	end
end
