...
function tamañoDePaso(t0, tf, N)
    if  !(N > 0)                          # Verificar si N es un entero positivo
        error("N debe ser un entero positivo")
    end
    h = (tf - t0) / N                                       # Calcular el tamaño de paso h
    return h
end

# ╔═╡ 0d89d182-9f6c-43df-ba1d-c9a48df64f44
tamañoDePaso(5, 7, 8)

# ╔═╡ c84d2b95-07a4-45f8-8e7e-e372acc89cb8
md" **Ejercicio** Crea una función `arregloUniforme` que tome los tres parámetros del **Ejercico** anterior y devuelva un arreglo uniforme de números desde `t0` hasta `tf` con el tamaño de paso dado por `tamañoDePaso` "

# ╔═╡ b0eb99e9-978a-4914-90a9-e05c73115e73
function arregloUniforme(t0, tf, N)
    h = tamañoDePaso(t0, tf, N)  # Obtener el tamaño de paso utilizando la función tamañoDePaso

    arreglo = [t0 + (i * h) for i in 0:N]  # Crear el arreglo uniforme de números desde t0 hasta tf

    return arreglo
end


# ╔═╡ 70cce824-8417-45b3-825e-ce4f23523044
arregloUniforme(5, 7, 8)
...
* `t` es un arreglo uniforme de "tiempos" de la forma $\big\{t_0 + ih \mid h\in\mathbb{R}^+, i\in\mathbb{N}^+, \ 0\leq i\leq N\big\}$,
...
function euler(g, xt0, t)
    N = length(t)  # Obtener el número de elementos en el arreglo t
    x = [xt0]  # Inicializar el arreglo con la condición inicial xt0
    
    for i in 1:N-1  # Iterar desde 1 hasta N-1 (excluyendo el último índice)
        ti = t[i]  # Obtener el tiempo ti en la posición i del arreglo t
        xti = x[i]  # Obtener la aproximación anterior xti en la posición i del arreglo x
        h = tamañoDePaso(ti, t[i+1], N)  # Calcular el tamaño de paso h utilizando la función tamañoDePaso
        xti1 = paso_euler(ti, xti, g, h)  # Calcular la aproximación de x(ti+1) usando el método de Euler
        push!(x, xti1)  # Agregar la aproximación al arreglo x
    end
    
    return x  # Devolver el arreglo con las aproximaciones de x(t_i)
end
...
$x'(t)= g(t,x(t))$  
Si  g solo depende del tiempo entonces

$x'(t)= g(t)$

y ademas 

$x(t_{i+1}) \approx x(t_i) + hg(t_i)$
...
md""" **Ejercicio** Utiliza tu implementación del método de Euler para solucionar el problema de condiciones iniciales $(3),(4)$. Grafica tu resultado junto con la gráfica de la solución analítica encontrada en el **Ejemplo** de la sección "Condiciones iniciales" y haz interactivo el parámetro $N$ para observar cómo cambia la aproximación que da tu solución numérica de la solución analítica en función del número de puntos utilizados en el intervalo $[t_0,t_f]$. """

# ╔═╡ 80b45143-b908-4f89-bb6f-1cb500529ea9
N = @bind N Slider(1:0.1: 10, default = 1)
#= Creamos una variable C, con valor asignado por un "slider" interactivo que se mueve en el rango -10:0.1:10.
Además, le asignamos este "slider" a la variable deslizador para poder cambiar el valor de C más adelante. =#

# ╔═╡ b8528a5a-8a75-4f49-8138-df888c53cb28
begin
	# Definir la función g que representa la ecuación (3)
	function g(t, x)
	    return t + x  # Ejemplo: g(t, x) = t + x
	end
	
	# Definir la condición inicial xt0
	xt0 = 1.0
	
	# Crear el arreglo uniforme de tiempos t utilizando las funciones anteriores
	t0 = 0.0
	tf = 1.0
	t = arregloUniforme(t0, tf, N)
	
	# Llamar a la función euler y obtener el arreglo de aproximaciones de x(t_i)
	x_aprox = euler(g, xt0, t)
	
	# Imprimir el arreglo de aproximaciones
	println("Aproximaciones de x(t_i): ", x_aprox)
end
...
begin
	...
	function solucion_analitica(t)
	    return t^2 + 1
	end
	
	# Obtener solución analítica
	x_analitica = solucion_analitica.(t)
	
	# Imprimir resultados
	println("Aproximación numérica: ", x_aprox)
	println("Solución analítica: ", x_analitica)
	
end

# ╔═╡ 69d5a0f7-3db6-4437-a909-e7679674aca2
N

# ╔═╡ 67a788ee-8004-4877-a733-3c258a8e303e
begin
	plot(t, x_analitica, label = "Solución analítica", linewidth = 2)
	scatter!(t, x_analitica, markersize = 4, color = :blue )
	scatter!(x, label = "condiciones iniciales", markersize = 4, color = :orange)
xlabel!("t")
ylabel!("x(t)")
title!("Método de Euler: Aproximación numérica vs. Solución analítica")
end
