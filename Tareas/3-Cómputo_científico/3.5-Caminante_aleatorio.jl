...
# ╔═╡ 7a6a0eb3-7786-4a07-adb8-e3c53b5b13f1
...
# ╔═╡ 7db6e4a2-ae38-4c95-8912-025c9569b6e3
numero= 5

# ╔═╡ 5813e9dc-12e4-4004-9f6c-479a33cf5672
rand()

# ╔═╡ 2eff46aa-0f3b-456b-be50-8b819642e641
rand(Int)

# ╔═╡ b3eb5530-1ec9-4d03-9999-f4de9383b2a4
rand(Bool)

# ╔═╡ 43cb6968-2a04-402a-8085-4f953c120f15
rand(numero)

# ╔═╡ df646b24-9e58-4843-aa56-ce56b6a35d62
rand(Int,numero)

# ╔═╡ 232bc560-d368-4fff-baa8-afe8822aef8a
rand(Bool,numero)
...
# ╔═╡ 4d2b2ffd-5af2-4e4f-81fc-cedf0e495822
function aleatorioUniforme(a, b)
    return a + (b - a) * rand()
end

# ╔═╡ 708c9392-c3e9-48fc-934c-00719aa8a8d6
md" `rand()` nos da números aleatorios entre 0 y 1, para tener nuestro intervalo de números tenemos primero que restar a-b esto nos dará la distancia de los números. Multiplicamos `rand()` por el intervalo lo que hará que modifiquemos el intervalo a la distancia de a-b. Aún falta que empiece en en a por lo que desplazamos el intervalo sumando a."
...
# ╔═╡ 9b83cffa-3e3d-4793-a3d6-768fdf33ff29
println(aleatorioUniforme(1,10))

# ╔═╡ 346ab456-b225-4731-9f26-35dd2f3d8b7b
begin
	
	function generarHistograma(a, b, num_valores)
	    valores = [aleatorioUniforme(a, b) for _ in 1:num_valores]
	    histogram(valores, bins=1, title="Histograma")
	end
	
	generarHistograma(1, 10, 1000)
...
# La función Bool nos da valores de 0 o 1 si lo multiplicamos por 2 tenemos 0 y 2 y si restamos 1 tenemos 1 y -1

# ╔═╡ c12c9786-75c2-49a0-9318-18a0c4465444
unPaso()
...
# ╔═╡ e3fc0df9-1073-4861-a85f-b3d4d7d25166
 variosPasos(10)
...
function caminataAleatoria(n)
    pasos = variosPasos(n)  # Llamamos a la función variosPasos para generar los pasos de la caminata
    posicion_actual = cumsum([0; pasos])  # Utilizamos la función cumsum para calcular la posición acumulada en cada paso
    return posicion_actual
end

# ╔═╡ 0a451abf-7807-4658-86c1-167510501a13
...
function graficaCaminata(caminata)
    n = length(caminata)
    pasos = 0:n
    scatter(pasos, caminata, xlabel="Número de pasos", ylabel="Posición", title="Caminata Aleatoria")
end

# ╔═╡ d4bfbc93-9f82-4bf4-b1d7-a9c77b8e9ee4
begin
	n2 = 4  # Definir el número de pasos para la caminata aleatoria
	caminata = caminataAleatoria(n2)  # Generar la caminata aleatoria
	graficaCaminata(caminata)  # Generar y mostrar la gráfica de la caminata aleatoria
end
...
# ╔═╡ 72ad9c4c-daaa-4d59-b742-faf0a56dac4d
begin
    function graficaCaminata!(caminatas)
        n = size(caminatas, 2) - 1
        steps = 0:n
        plot(steps, caminatas', xlabel="Número de pasos", ylabel="Posición", title="Caminatas Aleatorias", linestyle=:dash)
    end
    num_caminatas = 5  # Número de caminatas aleatorias

    caminatas = zeros(n2+1, num_caminatas)  # Crear un arreglo para almacenar las caminatas aleatorias

    for i in 1:num_caminatas
        caminatas[:, i] = caminataAleatoria(n2)  # Generar cada caminata aleatoria y almacenarla en el arreglo
    end

    graficaCaminata!(caminatas)  # Generar y mostrar la gráfica con líneas punteadas para las caminatas
end
...
function animaCaminata(caminata)
    ...
    anim = @animate for i in 1:length(steps)
        plot(steps[1:i], caminata[1:i], xlabel="Número de pasos", ylabel="Posición", title="Caminata Aleatoria", linestyle=:dash)
    end
    
    gif(anim, "caminata_aleatoria.gif", fps = 1)
end

# ╔═╡ 592150c2-2d16-4c79-8eb8-d420869856d8
begin
...
Utiliza tu función `aleatorioUniforme` para generar números aleatorios en el intervalo de numeros reales $[-1,1]$ y suma números generados por esta función a las posiciones horizontales y verticales para simular un paso continuo en dos dimensiones."""

# ╔═╡ d1c6a096-583d-4f85-b2cd-89d227311c1d
function caminataAleatoria2D(n)
    # Creamos arreglos vacíos para almacenar las posiciones horizontales y verticales
    posiciones_x = [0.0]
    posiciones_y = [0.0]
    
    # Realizamos la caminata aleatoria en 2D
    for i in 1:n
        # Generamos dos números aleatorios en el rango [-1, 1]
        paso_x = aleatorioUniforme(-1, 1)
        paso_y = aleatorioUniforme(-1, 1)
        
        # Calculamos las nuevas posiciones sumando los pasos a las posiciones anteriores
        nueva_posicion_x = posiciones_x[end] + paso_x
        nueva_posicion_y = posiciones_y[end] + paso_y
        
        # Agregamos las nuevas posiciones a los arreglos
        push!(posiciones_x, nueva_posicion_x)
        push!(posiciones_y, nueva_posicion_y)
    end
    
    # Devolvemos un arreglo que contiene las posiciones horizontales y verticales
    return [posiciones_x, posiciones_y]
end

# ╔═╡ a348a62b-9287-4e52-bab2-ec3e029fc44b
caminataAleatoria2D(3)
...
begin
	function graficaCaminata2D(caminata2d)
	    # Obtener las posiciones horizontales y verticales de la caminata
	    posiciones_horizontales = caminata2d[1]
	    posiciones_verticales = caminata2d[2]
	
	    # Crear la gráfica bidimensional
	    plot(posiciones_horizontales, posiciones_verticales, linestyle=:dash, 
	         xlabel="Posición horizontal", ylabel="Posición vertical", 
	         title="Caminata Aleatoria 2D")
	end
	
	n3 = 100  # Número de pasos
	caminata2d = caminataAleatoria2D(n3)  # Generar caminata aleatoria en dos dimensiones
	graficaCaminata2D(caminata2d)  # Crear y mostrar la gráfica de la caminata
...
begin
	function graficaCaminata2D!(caminatas2d)
	    # Crear la gráfica bidimensional
	    plot(title="Caminatas Aleatorias 2D", legend=:none)
	
	    for caminata2d in caminatas2d
	        # Obtener las posiciones horizontales y verticales de la caminata
	        posiciones_horizontales = caminata2d[1]
	        posiciones_verticales = caminata2d[2]
	
	        # Agregar la trayectoria de la caminata a la gráfica
	        plot!(posiciones_horizontales, posiciones_verticales, linestyle=:dash)
	    end
	...
	n4 = 100  # Número de pasos
	begin
	    numerodecaminatas = 5  # Número de caminatas aleatorias
	    caminatas2d = [caminataAleatoria2D(n4) for _ in 1:numerodecaminatas]  # Generar caminatas aleatorias en dos dimensiones
	    graficaCaminata2D!(caminatas2d)  # Crear y mostrar la gráfica de las caminatas
	end
end
...
begin
	function animaCaminata2D(caminata2D)
	    n = length(caminata2D[1]) - 1
	    steps = 0:n
	    
	    anim = @animate for i in 1:length(steps)
	        plot(caminata2D[1][1:i], caminata2D[2][1:i], xlabel="Posición Horizontal", ylabel="Posición Vertical", title="Caminata Aleatoria 2D", linestyle=:dash)
	    end
	    
	    gif(anim, "caminata_aleatoria_2D.gif", fps = 10)
	end
	
end

# ╔═╡ 43abe0f9-1822-497a-8da1-89b54e0fb7eb
begin
	caminata2D = caminataAleatoria2D(100) 
	animaCaminata2D(caminata2D)
...
begin
	function caminataAleatoria3D(n)
	    ...
	    for i in 2:(n+1)
	        dx, dy, dz = randn(3)  # Generar tres números aleatorios según una distribución normal
	        x[i] = x[i-1] + dx    # Paso en la dimensión x
	        y[i] = y[i-1] + dy    # Paso en la dimensión y
	        z[i] = z[i-1] + dz    # Paso en la dimensión z
	    end
	
	    return (x, y, z)
	end
	
	function graficaCaminata3D(caminata3D)
	    x, y, z = caminata3D
	
	    plot3d(x, y, z, xlabel="Posición X", ylabel="Posición Y", zlabel="Posición Z", title="Caminata Aleatoria 3D", linestyle=:dash)
	end
	
	n = 1000  # Número de pasos
	caminata3D = caminataAleatoria3D(n)
	graficaCaminata3D(caminata3D)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "7e3f953780185311e7a05822bbf6c4c63c8219fc"
