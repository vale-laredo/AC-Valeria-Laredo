...
# ╔═╡ 7489e817-702a-44fe-9300-b8ec8baae356
using Plots, PlutoUI
...
function newtonDistancia(efe, efe′, x0, ϵ)
    if ϵ <= 0
        error("El valor de ϵ debe ser mayor que cero.")
    end
    
    x_anterior = x0  # Inicializamos la aproximación anterior con x0
    x_actual = x0 - (efe(x0) / efe′(x0))  # Calculamos la primera aproximación actual
    
    while abs(x_actual - x_anterior) > ϵ
        x_anterior = x_actual  # Actualizamos la aproximación anterior con la actual
        x_actual = x_actual - (efe(x_actual) / efe′(x_actual))  # Calculamos la nueva aproximación actual
    end
    
    return x_actual  # Devolvemos la aproximación de la raíz
end
...
# ╔═╡ 7fade0cb-468e-41f0-8462-01a97a0b9954
...
* `xo` en el rango `-10:0.1:10`,
* `h2` en `1000:1000:1000000`, y
* `n2` en `1:1000`,
...
# ╔═╡ 04605f5f-5f4f-4c46-894d-3d41ff51d551
f=cos

# ╔═╡ 861b7f33-6aec-4c9c-bd26-388dcd8156f9
function plot_function(x0, h, n)
	    x_vals = range(-10, stop = 10, length = 100)
	    y_vals = f.(x_vals)
	    plot(x_vals, y_vals, label = "f(x)")
	    for i in 0:n
	        x_i = newtonDN(f, x0, i, h)
	        y_i = f(x_i)
	        scatter!([x_i], [y_i], label = "Iteración $i")
	        plot!([x_i, x_i], [0, y_i], linestyle = :dash)
	    end
	    xlabel!("x")
	    ylabel!("f(x)")
	    title!("Método de Newton con Derivada Numérica")
	end

# ╔═╡ 4fedb92a-11d3-4131-8e56-92d4f5d28847
@bind x0 Slider(-10:0.1:10, default=0.5)

# ╔═╡ 793cc3e6-f541-46a7-8ea7-3bf1c14e707c
@bind h Slider(1000:1000:1000000, default=0.5)

# ╔═╡ 4b262173-6924-4a84-95ae-192fa4a564cf
@bind n Slider(1:10000, default=0.5)

# ╔═╡ 38353717-f916-4180-b3cf-ca0eaac58d65
plot_function(x0, h, n)
