...
# ╔═╡ 70295666-0caf-11ee-20a5-19a0252fd757
using LaTeXStrings

# ╔═╡ 8f21a42e-4f07-4428-aacb-5a618774f475
using Plots, PlutoUI

# ╔═╡ 306713cf-526b-43a1-87f0-17328be320ab
md"# Modelo de Hodgkin-Huxley
...
# ╔═╡ affdc12a-a29f-4eaa-bd08-18229aa06a0a
md"###### Ejercicio.
...
# ╔═╡ af4f8b3b-3975-440c-b9d9-6d2f29f9b3e4
begin
	## Programa las función alfa_n como función del voltaje V.
	## Necesitarás definir un caso especial cuando V - 10 < epsilon
	...
	function alpha_n(V)
	epsilon = 1e-10                                              #Elegimos una epsilon muy pequeña
	    if V-10 < epsilon                                        #Como V=10 esta indefinida y ya sabemos el valor de su límite lo agregamos
	        return 0.1 
	    else 
	        return (0.01 * (10 - V)) / (exp((10 - V) / 10) - 1)  #Escribimos alpha
	        return 0.1 
	    end
	end
...
# ╔═╡ 3ee34fe6-06e4-4597-811e-1d9ca7f698cc
begin
...
# ╔═╡ 839abb05-9e99-48ed-a3f9-007ab302c165
begin
	# Funciones para la compuerta de sodio (Na)
	function alpha_m(V)
	...
	# Funciones para calcular tau_inf y n_inf
	...
	function plotParameters()
	    ...
	    plot(
	        ...
	        p2,
	        layout=(1, 2),
	        title=["Tasas de transición" "Constantes temporales"],
	        xlabel=[L"V\ con\ respecto\ a\ V_r\ (mV)" L"V\ con\ respecto\ a\ V_r\ (mV)"],
...
# ╔═╡ 23c41a2a-5503-4e54-9521-0f2367512b22
md"## Simulación con el método numérico de Euler
...
# ╔═╡ 768694dd-8909-4024-a8ed-1e25f2bb318a
md"Se obtienen fórmulas análogas para $m$ y $h$.  Todas juntas pueden ser escritas en forma matricial:
...
# ╔═╡ 2d254061-3ff9-41cf-a6e5-8481b277bc80
md"Se debe realizar el mismo procedimiento con la ecuación diferencial para el voltaje.  Para simplificar la notación, introduzcamos:
...
# ╔═╡ be552e8c-114e-4ede-a456-15b50dd67c83
md"Utilizando el método de Euler:
...
# ╔═╡ 33211c10-1c49-42c6-a8a4-1d02928b1c26
begin
	# NOTA
...
# ╔═╡ 6a96590e-482a-45aa-9957-77a6be67d717
begin
	# Funciones auxiliares para calcular las constantes de velocidad y las constantes temporales
	function tau_n(V)
	...
	function EulerHodkinHuxley(parametros)
	    ...
	    tiempo = range(0, maxT, step=DeltaT)
	    V = similar(tiempo)
	    n = similar(tiempo)
	    m = similar(tiempo)
	    h = similar(tiempo)
	    ...
	    V[1] = V0
	    n[1] = alpha_n(V0) / (alpha_n(V0) + beta_n(V0))
	    m[1] = alpha_m(V0) / (alpha_m(V0) + beta_m(V0))
	    h[1] = alpha_h(V0) / (alpha_h(V0) + beta_h(V0))
	    tauN[1] = tau_n(V0)
	    tauM[1] = tau_m(V0)
	    tauH[1] = tau_h(V0)
	
	    # Cálculo de los valores en cada paso de tiempo
	    for i = 1:Nt-1
	        # Cálculo de las constantes de velocidad y constantes temporales
	        alphaN = alpha_n(V[i])
	        betaN = beta_n(V[i])
	        alphaM = alpha_m(V[i])
	        betaM = beta_m(V[i])
	        ...
	        # Cálculo de la corriente iext y corriente total Itotal
	        t = tiempo[i]
	        iext = makeIext(t)  # Función externa definida previamente
	        Itotal = iext(t) - GK[i] * (V[i] - EK) - GNa[i] * (V[i] - ENa) - gL * (V[i] - EL)
	
	        # Cálculo de los valores en el siguiente paso de tiempo utilizando el método de Euler
	        V[i+1] = V[i] + (DeltaT / Cm) * Itotal
	        n[i+1] = n[i] + (DeltaT / tauN[i]) * (alphaN * (1 - n[i]) - betaN * n[i])
	        m[i+1] = m[i] + (DeltaT / tauM[i]) * (alphaM * (1 - m[i]) - betaM * m[i])
	        h[i+1] = h[i] + (DeltaT / tauH[i]) * (alphaH * (1 - h[i]) - betaH * h[i])
	    end
...
# ╔═╡ e62e04a1-6c8f-4d8f-be6c-af2c9b77a6ff
simulaHodkinHuxley(0, 10, 30, 10)

# ╔═╡ a1392c7e-4cb1-4940-af80-e5904925f83f
simulaHodkinHuxley(0, 10, 80, 10)

# ╔═╡ 81ecb92d-1c57-4a88-8898-4e663cbe6126
md"# Comentarios y resultados"

# ╔═╡ dd509eee-f9e3-4642-abcb-90ce3ba7b74f
md"Aunque no salieron las últimas graficas observando las imagenes proporcionadas podemos concluir que el modelo de Hodgkin-Huxley describe cómo se generan y propagan los potenciales de acción. A partir de las ecuaciones del modelo de Hodgkin-Huxley, sabemos que el modelo describe cómo los canales de sodio y potasio en la membrana neuronal afectan su comportamiento. La membrana es más permeable al potasio que al sodio, ya que su potencial de reposo o sea cuando el voltaje relativo en la membrana tiende a cero, se acerca al potencial de equilibrio del potasio.

En resumen, podemos decir que es una herramienta fundamental en la neurociencia computacional y nos permite comprender mejor los procesos eléctricos en las neuronas y su relación con la actividad cerebral."
